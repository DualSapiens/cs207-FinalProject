...
``therapy_planner`` can be installed with ``pip install therapy_planner`` and is suitable for an environment running Python 3. The sections below outline some of the basic concepts of IMRT, detail the optimization routine adapted for ``gradpy`` that is at the core of ``therapy_planner``, and present several example demos to get started.
